...
def evaluate(gold, predict, db_dir, etype, kmaps, plug_value, keep_distinct, progress_bar_for_each_datapoint):
    ...
    entries = []
    scores = {}
    error_indices = []  # Track error indices
    ...
    for i, (p, g) in enumerate(zip(plist, glist)):
        ...
        for idx, pg in enumerate(zip(p, g)):
            p, g = pg
            # Extract original index and SQL from pred
            if len(p) >= 2:
                original_idx = int(p[0])
                p_str = p[1]
            else:
                original_idx = i  # Fallback to sequential if no index
                p_str = p[0]
            ...
            if etype in ["all", "exec"]:
                ...
                if exec_score:
                    ...
                    error_indices.append(original_idx)  # Track original index
    ...
    # Write error indices to file
    if error_indices:
        with open('results/testsuite_error_indices.txt', 'w') as f:
            f.write(','.join(map(str, sorted(set(error_indices)))))
        print(f"\nWrote {len(set(error_indices))} error indices to 'results/testsuite_error_indices.txt'.")
        print("To re-run them: python src/main.py --indices results/testsuite_error_indices.txt")
